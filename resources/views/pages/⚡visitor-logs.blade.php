...
<div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
                        ...
                        <flux:table.row :key="$log->id">
                            <flux:table.cell variant="strong">
                                <div class="flex items-center gap-3">
                                    @if ($log->photo)
                                        <img src="{{ $log->photo }}" alt="" class="h-9 w-9 shrink-0 cursor-pointer rounded-full object-cover border border-neutral-200 transition-opacity hover:opacity-80 dark:border-neutral-700" x-on:click="previewPhoto = $event.target.src">
                                    @else
    ...
    {{-- View Visitor Modal --}}
    <flux:modal wire:model="showViewModal" name="view-visitor-log" class="min-w-sm">
            ...
            <div class="mt-6 space-y-4">
                @if ($this->viewingVisitor->photo)
                    <div class="flex justify-center">
                        <img src="{{ $this->viewingVisitor->photo }}" alt="Visit selfie" class="h-24 w-24 rounded-full object-cover border border-neutral-200 dark:border-neutral-700">
                    </div>
                @elseif ($this->viewingVisitor->visitor->photo)
                    <div class="flex justify-center">
    ...
    <flux:modal wire:model="showDeleteModal" name="delete-visitor-log" class="min-w-sm">
        @if ($this->deletingVisitor)
            <flux:heading size="lg">Delete Visit Record</flux:heading>
            <flux:text class="mt-2">
                Are you sure you want to delete the visit record for <strong>{{ $this->deletingVisitor->visitor->name }}</strong>?
